  ...
  return (
    <main className="max-w-6xl mx-auto px-8 py-12">
      <div className="flex items-center justify-between mb-10">
        <h2 className="text-4xl font-extrabold tracking-tight text-slate-900">Your Cart</h2>
      ...
      {!cart || cart.items?.length === 0 ? (
        <div className="text-center py-32 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
          <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
          </div>
          <p className="text-slate-500 font-semibold text-lg mb-6">Your shopping bag is empty</p>
          <a href="/" className="inline-flex items-center justify-center px-8 py-3 bg-black text-white rounded-2xl font-bold hover:bg-slate-800 transition-all active:scale-95">
            Continue Shopping
          </a>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            {cart.items.map((item) => (
              ...
              >
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-slate-900 group-hover:text-black transition-colors">{item.product.name}</h3>
                  <p className="text-slate-400 font-bold text-sm mt-1">₹{item.product.price.toLocaleString()}</p>
                </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-slate-50 border border-slate-200 p-8 rounded-[2rem] sticky top-32">
              <h3 className="text-xl font-extrabold text-slate-900 mb-8 tracking-tight">Order Summary</h3>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex justify-between text-sm font-medium text-slate-500">
                  <span>Shipping</span>
                  <span className={shipping === 0 ? "text-green-600 font-bold" : "text-slate-900 font-bold"}>
